...
# Adapted from Calculating Mean (Expected) Flow Frequency Cruve Using Bulletin 17C Conf.xlsm from david.a.margo@usace.army.mil
# 1/21/2019
...
if (Extrapolate == "No") {
    for (j in 1:(length(CLz))){
        #Create a function for linear interpolating the AEPz using logQ and the corresponding AEPz from table 2
        f<-approxfun(df1[1:dim(df1)[1],j],AEPz[1:length(AEPz)])

        for (i in 1:(length(Q))){
            # If the binned discharge is less than the minimum discharge for a specific confidence interval than set the estimated AEPz equal to the value for the minimum discharge at that confidence interval
            if (Q[i]<min(df1[1:dim(df1)[1],j],na.rm=TRUE)) df2[i,j]<-AEPz[length(AEPz)]

            if ((Q[i]>=min(df1[1:dim(df1)[1],j],na.rm=TRUE)) & (Q[i]<=max(df1[1:dim(df1)[1],j],na.rm=TRUE))) df2[i,j]<-f(Q[i])
            
            #If the binned discharge is greater than the maximum discharge for a specific confidence interval than set the estimated AEPz equal to the value for the maximum discharge at that confidence interval
            if (Q[i]>max(df1[,j][1:dim(df1)[1]],na.rm=TRUE)) df2[i,j]<-AEPz[1]
...
#   it cant be done with approxfun()
    ...
    for (j in 1:(length(CLz))){
        ...
        lowmodel<-lm(y ~ x, data = data.frame(y = c(AEPz[length(AEPz)-1],AEPz[length(AEPz)]), x = c(df1[dim(df1)[1]-1,j],df1[dim(df1)[1],j])))
